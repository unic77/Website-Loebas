import { useState } from 'react'
import { Dialog } from '@headlessui/react'
import { Bars3Icon, XMarkIcon } from '@heroicons/react/24/outline'
import './TopNavBar.css'
import { Link, Outlet, useLocation } from 'react-router-dom'


const navigation = [
  { name: 'Projects', href: '/projects', bool: true},
  { name: 'Company', href: '/company', bool: false },
  { name: 'Team', href: '/team', bool: false},
]

function onClick(){
  setMobileMenuOpen = useState(false)
}

export default function TopNavBar() {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)

  let location = useLocation();

  return (
      <div>
          <header className="absolute inset-x-0 top-0 z-50 shadow-lg">
            <div className="bg-white">
              <nav className="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
                <Link to="/">
                  <div className="-m-1.5 p-1.5">
                    <span className="sr-only">Your Company</span>
                    <img
                      className="h-8 w-auto"
                      src="/src/assets/logo.jpg"
                      alt=""
                    />
                  </div>
                </Link>
                <div className="flex lg:hidden">
                  <button
                    type="button"
                    className="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-gray-700"
                    onClick={() => setMobileMenuOpen(true)}
                  >
                    <span className="sr-only">Open main menu</span>
                    <Bars3Icon className="h-6 w-6" aria-hidden="true" />
                  </button>
                </div>
                <div className="hidden lg:flex lg:gap-x-12 auto">
                  {navigation.map((item) => (
                    location.pathname.includes(item.href) ? 
                      (
                        <Link to={item.href} key={item.href}>
                          <div className="text-xl font-semibold leading-6 text-indigo-600">
                            {item.name}
                          </div>
                        </Link>
                      )
                      :
                      (
                        <Link to={item.href} key={item.href}>
                          <div className="text-base font-semibold leading-6 text-gray-900 hover:text-indigo-600">
                            {item.name}
                          </div>
                        </Link>
                      )
                  ))}
                </div>
              </nav>
              <Dialog as="div" className="lg:hidden" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
                <div className="fixed inset-0 z-50" />
                <Dialog.Panel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
                  <div className="flex items-center justify-between">
                    <Link to="/" onClick={onClick}>
                      <div className="-m-1.5 p-1.5">
                        <span className="sr-only">Your Company</span>
                          <img
                            className="h-8 w-auto"
                            src="https://tailwindui.com/img/logos/mark.svg?color=indigo&shade=600"
                            alt=""
                          />
                      </div>
                    </Link>
                    {/* <a href="#" className="-m-1.5 p-1.5">
                      <span className="sr-only">Your Company</span>
                      <img
                        className="h-8 w-auto"
                        src="https://tailwindui.com/img/logos/mark.svg?color=indigo&shade=600"
                        alt=""
                      />
                    </a> */}
                    <button
                      type="button"
                      className="-m-2.5 rounded-md p-2.5 text-gray-700"
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      <span className="sr-only">Close menu</span>
                      <XMarkIcon className="h-6 w-6" aria-hidden="true" />
                    </button>
                  </div>
                  <div className="mt-6 flow-root">
                    <div className="-my-6 divide-y divide-gray-500/10">
                      <div className="space-y-2 py-6">
                        {navigation.map((item) => (
                            item.href === location.pathname ? 
                              (
                                <Link to={item.href} key={item.href} onClick={onClick}>
                                  <text className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7">
                                    {item.name}
                                  </text>
                                </Link>
                              )
                              :
                              (
                                <Link to={item.href} key={item.href} onClick={onClick}>
                                  <text className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900 hover:text-indigo-600">
                                    {item.name}
                                  </text>
                                </Link>
                              )
                          // <a
                          //    key={item.name}
                          //    href={item.href}
                          //    className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50"
                          //  >
                          //    {item.name}
                          // </a>
                        ))}
                      </div>
                    </div>
                  </div>
                </Dialog.Panel>
              </Dialog>
            </div>
          </header>

        <div className='md:container'>
          <Outlet/>
        </div>
      </div>
  )
}
